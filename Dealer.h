...
/*
 * BONUS PROBLEM (10PTS): Finish the merge algorithm for the Merge Sort with the comparator
 *
...
class CarComparator {
    public:
        int operator()(Car a, Car b){
  				if(a.getModel() != b.getModel()) return a.getModel().compare(b.getModel());
        	if(a.getBrand() != b.getBrand()) return a.getBrand().compare(b.getModel());
